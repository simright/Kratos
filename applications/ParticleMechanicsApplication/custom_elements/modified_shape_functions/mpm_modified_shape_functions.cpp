...
namespace Kratos
{
    ...
    /// MPMModifiedShapeFunctions implementation
    /// Default constructor
    MPMModifiedShapeFunctions::MPMModifiedShapeFunctions(const GeometryPointerType pInputGeometry, const Vector& rNodalDistances) :
        mpInputGeometry(pInputGeometry), mNodalDistances(rNodalDistances) {};

    /// Destructor
    MPMModifiedShapeFunctions::~MPMModifiedShapeFunctions() {};

    /// Turn back information as a string.
    std::string MPMModifiedShapeFunctions::Info() const {
        return "Modified shape functions computation for embedded MPM base class.";
    };

    /// Print information about this object.
    void MPMModifiedShapeFunctions::PrintInfo(std::ostream& rOStream) const {
        rOStream << "Modified shape functions computation for embedded MPM base class.";
    };

    /// Print object's data.
    void MPMModifiedShapeFunctions::PrintData(std::ostream& rOStream) const {
    ...
    const DivideGeometry::Pointer MPMModifiedShapeFunctions::pGetSplittingUtil() const {
        KRATOS_ERROR << "Trying to retrieve the splitting utility from the MPM modified shape functions base class. \n" <<
    ...
    const MPMModifiedShapeFunctions::GeometryPointerType MPMModifiedShapeFunctions::GetInputGeometry() const {
        return mpInputGeometry;
    };

    // Returns the nodal distances vector.
    const Vector& MPMModifiedShapeFunctions::GetNodalDistances() const {
        return mNodalDistances;
    };

    // Sets the condensation matrix to transform the subdivision values to entire element ones.
    void MPMModifiedShapeFunctions::SetCondensationMatrix(
    ...
    void MPMModifiedShapeFunctions::ComputeValuesOnOneSide(
    ...
    void MPMModifiedShapeFunctions::ComputeFaceValuesOnOneSide(
    ...
    void MPMModifiedShapeFunctions::ComputeFaceNormalOnOneSide(
    ...
    void MPMModifiedShapeFunctions::ComputeEdgeIntersectionValuesOnOneSide(
        const Matrix &rPmatrix,
